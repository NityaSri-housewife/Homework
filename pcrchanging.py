...
def determine_pcr_level(pcr_value):
    ...
    elif pcr_value >= 2: return "Strong Support", "Strike price +10"
    elif pcr_value >= 1.9: return "Support", "Strike price +20"
    elif pcr_value >= 1.2: return "Support", "Strike price -20"
    elif 0.81 <= pcr_value <= 1.19: return "Neutral", "0"
    elif pcr_value <= 0.8 and pcr_value > 0.59: return "Resistance", "Strike price +20"    
    elif pcr_value <= 0.5 and pcr_value > 0.4: return "Resistance", "Strike price +15"
    elif pcr_value <= 0.4 and pcr_value > 0.3: return "Resistance", "Strike price -10"
...
# ========== PCR & OI HISTORY ==========
def record_pcr(strike, pcr):
    supabase.table("pcr_history").insert({
        "strike": strike,
        "PCR": pcr
    }).execute()

def fetch_recent_pcr(strike, n=3):
    rows = supabase.table("pcr_history").select("*").eq("strike", strike).order("timestamp", desc=True).limit(n).execute().data
    return [r["PCR"] for r in reversed(rows)]  # oldest first

def record_oi(strike, ce, pe):
    supabase.table("oi_history").insert({"strike": strike, "CE": ce, "PE": pe}).execute()

def fetch_recent_oi(strike, n=3):
    rows = supabase.table("oi_history").select("*").eq("strike", strike).order("timestamp", desc=True).limit(n).execute().data
    ce_build = sum([r['CE'] for r in rows])
    pe_build = sum([r['PE'] for r in rows])
    return ce_build, pe_build
...
def process_signals(results, underlying_price):
    ...
    for row in results:
        ...
        # Record PCR & OI history
        record_pcr(row["Strike"], row["PCR"])
        record_oi(row["Strike"], row["changeinOpenInterest_CE"], row["changeinOpenInterest_PE"])
        recent_pcr = fetch_recent_pcr(row["Strike"])
        ce_build, pe_build = fetch_recent_oi(row["Strike"])

        # Determine dynamic entry threshold
        threshold = 4
        if (len(recent_pcr) >= 2 and recent_pcr[-1] > recent_pcr[0]) or ce_build > 0 or pe_build > 0:
            threshold = 2  # lower threshold for slow moves

        # Entry Signals
        if in_zone and total_score >= threshold and ask_qty_bias=="Bullish" and chg_oi_bias=="Bullish":
        ...
        elif in_zone and total_score <= -threshold and ask_qty_bias=="Bearish" and chg_oi_bias=="Bearish":
...
def main():
    ...
    st_autorefresh(interval=23 * 10000, key="data_refresh")
