...
def process_historical_data(data, interval):
    ...
    if 'timestamp' in data and len(data['timestamp']) > 0:
        # Check if the first timestamp is in milliseconds (typically 13 digits)
        if data['timestamp'][0] > 1e12:  # Likely milliseconds
            timestamp_unit = 'ms'
        else:  # Likely seconds
            timestamp_unit = 's'
    else:
        timestamp_unit = 's'  # Default to seconds
    
    df = pd.DataFrame({
        'timestamp': pd.to_datetime(data['timestamp'], unit=timestamp_unit).dt.tz_localize('UTC').dt.tz_convert(ist),
        'open': data['open'],
        'high': data['high'],
        'low': data['low'],
        'close': data['close'],
