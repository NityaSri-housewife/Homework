...
def detect_market_phase(ce_oi_change, pe_oi_change, atm_straddle_trend, price_action):
    """
    Detect the current market phase based on 2025 evolved market behavior patterns.
    
    Parameters:
    - ce_oi_change: Call OI percentage change (float)
    - pe_oi_change: Put OI percentage change (float)
    - atm_straddle_trend: ATM Straddle Premium trend ('expanding' or 'decaying')
    - price_action: Current price action ('up', 'down', or 'flat')
    
    Returns:
    - str: Market phase description
    """
    # Determine magnitude of OI changes (using absolute values)
    ce_oi_strong = abs(ce_oi_change) > 10  # Threshold for strong OI change
    pe_oi_strong = abs(pe_oi_change) > 10
    ce_oi_burst = abs(ce_oi_change) > 25   # Threshold for burst OI change
    pe_oi_burst = abs(pe_oi_change) > 25
    
    # Sideways / Algo Chop
    if (ce_oi_change > 0 and pe_oi_change > 0 and 
        atm_straddle_trend == 'decaying' and 
        price_action == 'flat'):
        return "Sideways (Algo Chop)"
    
    # Breakout Brewing
    elif (ce_oi_change < 0 and pe_oi_change < 0 and 
          atm_straddle_trend == 'expanding' and 
          price_action in ['up', 'down']):
        return "Breakout Brewing"
    
    # Uptrend (Gamma Squeeze)
    elif (ce_oi_change < 0 and pe_oi_change > 0 and 
          atm_straddle_trend == 'expanding' and 
          price_action == 'up'):
        return "Uptrend (Gamma Squeeze)"
    
    # Downtrend (Gamma Crush)
    elif (ce_oi_change > 0 and pe_oi_change < 0 and 
          atm_straddle_trend == 'expanding' and 
          price_action == 'down'):
        return "Downtrend (Gamma Crush)"
    
    # Trap Zone
    elif ((ce_oi_change > 0 and price_action == 'up') or 
          (pe_oi_change > 0 and price_action == 'down')) and (
          atm_straddle_trend not in ['expanding', 'decaying'] or 
          price_action in ['up', 'down']):
        return "Trap Zone (Whipsaws Likely)"
    
    # Call Accumulation
    elif (ce_oi_burst and ce_oi_change > 0 and 
          abs(pe_oi_change) < 5 and 
          price_action == 'flat'):
        return "Call Accumulation (Bullish Setup)"
    
    # Put Accumulation
    elif (pe_oi_burst and pe_oi_change > 0 and 
          abs(ce_oi_change) < 5 and 
          price_action == 'flat'):
        return "Put Accumulation (Bearish Setup)"
    
    # Call Distribution
    elif (ce_oi_change < -20 and 
          abs(pe_oi_change) < 5 and 
          price_action == 'up'):
        return "Call Distribution (Resistance Breaking)"
    
    # Put Distribution
    elif (pe_oi_change < -20 and 
          abs(ce_oi_change) < 5 and 
          price_action == 'down'):
        return "Put Distribution (Support Breaking)"
    
    # Default case if none of the patterns match
    return "Market Phase: Undefined (No Clear Pattern)"
...
def analyze():
    ...
    try:
        ...
        # === 2025 Market Phase Detection ===
        # Calculate ATM straddle trend (simplified logic)
        atm_straddle_trend = 'expanding' if (sum(item['CE']['impliedVolatility'] for item in records if 'CE' in item) + 
                                           sum(item['PE']['impliedVolatility'] for item in records if 'PE' in item)) > 0 else 'decaying'
        
        # Determine price action
        if len(st.session_state.price_data) >= 2:
            last_prices = st.session_state.price_data['Spot'].tail(2).values
            if last_prices[1] > last_prices[0] * 1.002:  # 0.2% increase
                price_action = 'up'
            elif last_prices[1] < last_prices[0] * 0.998:  # 0.2% decrease
                price_action = 'down'
            else:
                price_action = 'flat'
        else:
        ...
        market_phase = detect_market_phase(
            ...
            atm_straddle_trend=atm_straddle_trend,
            price_action=price_action
        )
        
        # Add phase explanation mapping
        phase_explanations = {
            "Sideways (Algo Chop)": "CE & PE OI increasing, ATM premium decaying - price stuck in range",
            "Breakout Brewing": "CE & PE OI decreasing, ATM premium expanding fast - volatility coming",
            "Uptrend (Gamma Squeeze)": "CE OI down (shorts covering) + PE OI up (fresh writing), price rising",
            "Downtrend (Gamma Crush)": "CE OI up (fresh writing) + PE OI down (longs unwinding), price falling",
            "Trap Zone (Whipsaws Likely)": "Contradictory OI vs price movement - expect reversals",
            "Call Accumulation (Bullish Setup)": "Strong CE OI buildup while price stalls below resistance",
            "Put Accumulation (Bearish Setup)": "Strong PE OI buildup while price stalls above support",
            "Call Distribution (Resistance Breaking)": "Sharp CE OI drop as price pushes through resistance",
            "Put Distribution (Support Breaking)": "Sharp PE OI drop as price pushes through support"
        }

        # Display market phase
        st.markdown("## 🔮 2025 Market Phase Detection")
        phase_color = "green" if "Uptrend" in market_phase else "red" if "Downtrend" in market_phase else "orange"
        st.markdown(f"**Current Phase:** <span style='color:{phase_color};font-weight:bold;'>{market_phase}</span>", 
                   unsafe_allow_html=True)
        
        if market_phase in phase_explanations:
            st.caption(f"*Interpretation:* {phase_explanations[market_phase]}")
